export class NetworkEventsCleaner {
  ...
  static normalizeJson(jsonString) {
    try {
      ...
      const sorted = NetworkEventsCleaner.sortObject(obj);
  ...
  static sortObject(obj) {
    if (Array.isArray(obj)) {
      return obj.map(NetworkEventsCleaner.sortObject);
    } else if (obj !== null && typeof obj === 'object') {
      return Object.keys(obj)
        .sort()
        .reduce((res, key) => {
          res[key] = NetworkEventsCleaner.sortObject(obj[key]);
  ...
  static createComparableEntry(entry) {
    ...
    return JSON.stringify({
      ...rest,
      url: NetworkEventsCleaner.normalizeUrl(entry.url),
      responseURL: NetworkEventsCleaner.normalizeUrl(entry.responseURL),
      requestData: NetworkEventsCleaner.normalizeJson(entry.requestData),
      response: NetworkEventsCleaner.normalizeJson(entry.response),
    });
  }
  ...
  static removeDuplicates(events) {
    ...
    return events.filter((event) => {
      const key = NetworkEventsCleaner.createComparableEntry(event);
